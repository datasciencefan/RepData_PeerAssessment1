...
The mean and median of the total number of steps taken each day are `r meanSteps` and `r medianSteps`, respectively.
...
The 5-minute interval on average over all days in the dataset that contains the maximum number of steps is interval `r maxInt`.
...
The number of rows with any NAs is `r numNAs`.
...
Using the new dataset, the mean and median of the total number of steps taken each day are `r meanSteps2` and `r medianSteps2`, respectively.

Note that the mean and median for the raw dataset were `r meanSteps2` and `r medianSteps2. There is almost no difference between these values, so we conclude there is little impact (at least when calculating the mean and median) by filling in the missing values with their interval means.
